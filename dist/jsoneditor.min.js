...
this.options.image&&this.input.addEventListener("change",function(b){if(a.value!==b.target.value){a.uploader.value="";try{new URL(b.target.value)}catch(c){return a.setValue(b.target.value),a.resetValues(),void(a.preview&&(a.preview.innerHTML="<strong>Invalid url format</strong>"))}a.preview_value=b.target.value,a.setValue(b.target.value),a.refreshPreview(!0)}}),!this.schema.readOnly&&!this.schema.readonly){if(!this.jsoneditor.options.upload||"function"!=typeof this.jsoneditor.options.upload)throw"Upload handler required for upload editor";
// File uploader
